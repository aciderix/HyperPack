...
import { ExtractedFilesPanel } from './ExtractedFilesPanel';
...
interface ResultPanelProps {
  ...
  onDownloadExtractedFile?: (index: number) => void;
...
export function ResultPanel({
  ...
  onReset,
  extractedFiles,
  onDownloadExtractedFile,
  onExtractAll,
  onDownloadZip,
}: ResultPanelProps) {
  const isCompress = mode === 'compress';
  const isMultiFile = !isCompress && extractedFiles && extractedFiles.length > 0;
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      <div className="p-6 bg-hp-card border border-hp-border rounded-xl space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4 text-center divide-x divide-hp-border">
          <div>
            <div className="text-sm text-hp-muted mb-1">{isCompress ? 'Ratio' : 'Ratio'}</div>
      ...
      {/* Extracted files browser (HPK6 decompress) */}
      {isMultiFile && onDownloadExtractedFile && onExtractAll && onDownloadZip && (
        <ExtractedFilesPanel
          files={extractedFiles!}
          onDownloadFile={onDownloadExtractedFile}
